...
class CSP:
    def __init__(self):
        ...
        # amount of times backtrack is called (a counter variable)
        self.backtracks = 0

        # amount of times a value failed to be consistent (a counter variable)
        self.failed = 0
    ...
    def backtrack(self, assignment):
        ...
        for value in assignment[var]:
            ...
            newAssignment[var] = list(value)
            # Queue of arcs to be iterated through later
            queue = self.get_all_arcs()
            if self.inference(newAssignment, queue):
    ...
    def inference(self, assignment, queue):
        ...
        """
        # TODO: YOUR CODE HERE
        # queue is a queue of arcs
        while len(queue) != 0:
            xi, xj = queue.pop()
            if self.revise(assignment, xi, xj):     # check if assignment has been revised
                if len(assignment[xi]) == 0:
                    return False
                # iterate throught the neighbouring arcs, to add them to queue, to make sure value is consistent with all arcs
    ...
    def revise(self, assignment, i, j):
        ...
        # should not remove right away inside
        revised = False
        for domainValueX in assignment[i]:
            domainXfound = False
            for domainValueY in assignment[j]:
                if (domainValueX, domainValueY) in self.constraints[i][j]:
                    # if makes it inside here, then do not remove domainValue
                    domainXfound = True
            if not domainXfound:
                assignment[i].remove(domainValueX)
                revised = True
